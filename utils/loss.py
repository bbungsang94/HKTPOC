...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target box를 통해 해당 그리드가 물체 포함 유무를 설정

            n = b.shape[0]  # target의 개수
            if n:
        ...
        lbox *= self.hyp['box'] # 예측 box와 target box간의 차이에서 발생하는 loss
        lobj *= self.hyp['obj'] # 예측 box에 물체가 있는지 여부와 target box의 물체감지 유무에 따른 loss
        lcls *= self.hyp['cls'] # 예측 box가 예측한 class와 target box의 class 차이 에 따른 loss
        bs = tobj.shape[0]  # batch size

        loss = lbox + lobj + lcls # 각 loss를 다 더하고 batchsize를 곱하여 total loss와 각 loss값을 붙여 리턴
