...
def detect(det, tensor_image, visible=False):
    begin = time.time()
    dt_image, result = det.detection(tensor_image)
...
if __name__ == '__main__':
    ...
    for image_path in image_list:
        ...
        detected_image, info = detect(det=block_detector, tensor_image=image_tensor, visible=save_debug)

        image_blur = cv2.imread(os.path.join(blur_path, image_path), cv2.IMREAD_GRAYSCALE)
        overlay_image = image_handler.draw_boxes_info(image_blur, info)
        if save_debug:
        ...
        for i in range(boxes.shape[0]):
            ...
            if surface > largest_anchor['surface']:
                ...
                roi = image_blur[height_min:height_max, width_min:width_max]
        ...
        for i in range(boxes.shape[0]):
            ...
            multiple_gap = abs(largest_anchor['surface'] - multiple)
        ...
        deleted_image = image_handler.draw_boxes_info(image_blur, info)
        if save_debug:
