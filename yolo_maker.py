...
def main():
    ...
    label_path = os.path.join(det_config['raw_path'], det_config['label_folder'])
    ...
    make_folder_list = [det_config['raw_path'],
                        detected_path, overlay_path, deleted_path, anchor_path, label_path]
    ...
    for image_path in image_list:
        ...
        text_name = image_path.replace('.jpg', '.txt')
        f = open(os.path.join(label_path, text_name), 'w')
        ...
        for i in range(boxes.shape[0]):
            ...
            writing_data = '0 '
            writing_data += str(center_x) + ' ' + str(center_y) + ' ' + str(width / w) + ' ' + str(height / h)
            writing_data += '\n'
